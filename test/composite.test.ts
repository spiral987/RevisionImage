...
describe('flattenState (純TS合成)', () => {
  ...
  it('半透明レイヤは背景とブレンドされる（赤 a=128 over 白 → 255,127,127）', () => {
    ...
    leafAt(s, 0).buffer.data.set([255, 0, 0, 128], 0);
    const flat = flattenState(s);
  ...
  it('非整数オフセットでも丸めて合成される（translateプレビュー中の消失を防ぐ回帰テスト）', () => {
    const s = createInitialState(2, 1);
    leafAt(s, 0).buffer.data.set([255, 0, 0, 255], 0); // ローカル(0,0) を赤
    leafAt(s, 0).offsetX = 0.7; // ドラッグ中プレビュー相当の非整数オフセット
    const flat = flattenState(s);
  ...
  it('非表示レイヤは合成されない', () => {
    const s = createInitialState(1, 1);
    leafAt(s, 0).buffer.data.set([255, 0, 0, 255], 0);
    leafAt(s, 0).visible = false;
    const flat = flattenState(s);
